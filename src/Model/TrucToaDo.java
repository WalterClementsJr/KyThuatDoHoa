...
//        p = 2 * Dy - Dx;
    ...
    int x1 = x + w;
    int y1 = y + h;
    bresenhamLine(g,x, y, x1, y);
    bresenhamLine(g,x1, y, x1, y1);
    bresenhamLine(g,x, y1,x1, y1);
    bresenhamLine(g,x, y,x, y1);
}
    public void bresenhamCircle(Graphics g,int xc,int yc,int Radius) {
    int x, y, p;
    x = 0;
    y = Radius;
    p = 3 - 2 * Radius;
    putPixel(g,xc + Radius, yc + 0);
    putPixel(g,xc - Radius, yc - 0);
    putPixel(g,xc - 0, yc + Radius);
    putPixel(g,xc + 0, yc - Radius);
    while (x < y) {
        if (p < 0)
            p += 4 * x + 6;
   ...
   plot(g,xc, yc,Math.round(x),Math.round(y));
    ...
    public void veHinhTamGiacDeu(Graphics g, int xDinh,int yDinh, int canh) {
        ...
        xC = xA+canh/2;
        yC = yA+aH;
        bresenhamLine(g, xA, yA, xB, yB);
        bresenhamLine(g, xA, yA, xC, yC);
        bresenhamLine(g, xB, yB, xC, yC);
    }
//    public void dashedLine(Graphics g,int x1,int y1,int x2,int y2) {
...
//    for (x = x1; x <= x2; x++) {
//        dem++;
//        if (dem <= chieuDaiMoiDoan) putPixel(g,x, y);
//        else {
...
//    }
    ...
    public void dashDotLine(Graphics g, int x1, int y1, int x2, int y2) {
        int x, y, dx, dy, p, const1, const2, dem, chieuDaiMoiDoan, khoangCachMoiDoan;
        y = y1;
        dx = x2 - x1;
        dy = y2 - y1;
